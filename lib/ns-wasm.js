...
const VIEW = Object.freeze({
  size: 164, title: 0, width: 4, height: 8, framebufferWidth: 12,
  ...
  nativeWindow: 136, gpuDevice: 140,
  captureRequired: 144, captureStarted: 148,
});

// lib/ui.ns::ui_insets, four logical-point margins.
const UI_INSETS_SIZE = 32;
...
class NSBrowserRuntime {
  ...
  // application overrode or disabled it. Mirrors lib/src/ui.c.
  uiInsets(pointer) {
    const renderer = this.uiRenderer(pointer);
    const zero = { top: 0, right: 0, bottom: 0, left: 0 };
    if (!renderer || renderer.safeAreaDisabled) return zero;
    const insets = renderer.insets || this.safeAreaInsets();
    const [width, height] = this.uiSurfaceSize();
    return {
      top: insets.top + insets.bottom < height ? insets.top : 0,
      bottom: insets.top + insets.bottom < height ? insets.bottom : 0,
      left: insets.left + insets.right < width ? insets.left : 0,
      right: insets.left + insets.right < width ? insets.right : 0,
    };
  }
  ...
  ui(name, a) {
    ...
    if (name === "ui_renderer_create") {
      ...
      this.uiRenderers.set(pointer, { view: n(a[0]), commands: [], insets: null, safeAreaDisabled: false });
      return pointer;
    }
    ...
    if (name === "ui_canvas_width") {
      ...
      return Math.max(1, Math.round(this.uiSurfaceSize()[0] - insets.left - insets.right));
    }
    if (name === "ui_canvas_height") {
      const insets = this.uiInsets(a[0]);
      return Math.max(1, Math.round(this.uiSurfaceSize()[1] - insets.top - insets.bottom));
    }
    if (name === "ui_surface_width") return this.uiSurfaceSize()[0];
    if (name === "ui_surface_height") return this.uiSurfaceSize()[1];
    if (name === "ui_safe_area") {
      const insets = this.uiInsets(a[0]), pointer = this.allocStruct(UI_INSETS_SIZE), view = this.view();
      view.setFloat64(pointer, insets.top, true);
      view.setFloat64(pointer + 8, insets.right, true);
      view.setFloat64(pointer + 16, insets.bottom, true);
      view.setFloat64(pointer + 24, insets.left, true);
      return pointer;
    }
    if (name === "ui_safe_area_enabled") return this.uiRenderer(a[0])?.safeAreaDisabled ? 0 : 1;
    if (name === "ui_set_safe_area_enabled") {
      const renderer = this.uiRenderer(a[0]);
      if (renderer) renderer.safeAreaDisabled = !n(a[1]);
      return;
    }
    ...
    if (name === "ui_fill_surface") {
      const insets = this.uiInsets(a[0]), [width, height] = this.uiSurfaceSize();
      this.uiCommand(a[0], { kind: "fillRect", bleed: true, x: -insets.left, y: -insets.top, w: width, h: height, color: a[1] });
      return;
    }
    if (name === "ui_flush") {
      ...
      // Commands are recorded in the safe content space. The backgrounds that
      // ui_fill_surface records deliberately reach back out under the native
      // chrome, so they are painted before the content clip goes on; everything
      // else stays inside the safe area, as it does natively.
      const insets = this.uiInsets(a[0]);
      context.translate(insets.left, insets.top);
      for (const command of renderer.commands) {
        if (command.bleed) this.executeUICommand(context, command);
      }
      context.beginPath();
      context.rect(0, 0, width - insets.left - insets.right, height - insets.top - insets.bottom);
      context.clip();
      for (const command of renderer.commands) {
        if (!command.bleed) this.executeUICommand(context, command);
      }
    ...
    if (name === "ui_draw_text_vertical") {
      this.uiCommand(a[0], { kind: "textVertical", x: n(a[1]), y: n(a[2]), text: this.readString(a[3]), px: n(a[4]), color: a[5] }); return;
    }
    if (name === "ui_text_vertical_column_count") {
      return (this.readString(a[0]).match(/[^\s]+/g) || []).length ? this.readString(a[0]).split(/\n+/).filter(Boolean).length : 0;
    }
    if (name === "ui_text_vertical_max_run") {
      return Math.max(0, ...this.readString(a[0]).split(/\n/).map((line) => line.replace(/[ \t]/g, "").length));
    }
    if (name === "ui_text_vertical_column_width") return n(a[1]);
