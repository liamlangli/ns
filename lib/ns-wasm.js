...
// The root block src/ns_shader.c declares in generated WGSL, in bytes. WebGPU
// refuses a uniform binding smaller than the block the shader declares, and a
// pass may upload fewer words than that - the shader only reads the ones it
// wrote - so a root allocation is reserved and bound at the full block size and
// the upload fills its front. Keep this in step with
// NS_SHADER_ROOT_BLOCK_VEC4S.
const ROOT_BLOCK_BYTES = 256 * 16;
// The per-frame ring roots and other transient uploads are cut from. Roots are
// reserved at the full block size, so the ring is sized for a frame that binds
// a few hundred of them.
const FRAME_RING_BYTES = 4 * 1024 * 1024;
...
// A wheel event carries its travel in one of three units. Only the pixel mode
// matches what the native backends publish, so the other two are converted:
// one line is a text row, and one page is the canvas the wheel scrolled over.
const VIEW_WHEEL_LINE_PIXELS = 16;
const VIEW_WHEEL_PAGE_PIXELS = 800;
// A trackpad pinch reaches a page as a ctrl-held wheel rather than as a gesture
// of its own. This is the pixel travel that doubles the zoom factor, which is
// the browser convention every map view uses.
const VIEW_WHEEL_PINCH_PIXELS = 100;
...
class NSBrowserRuntime {
  ...
  frameOffset(id, offset, writing) {
    const framed = this.framedBuffers.get(Number(id));
    if (!framed) return offset;
    const section = this.frameIndex % 3;
    if (writing) {
      framed.wroteThisFrame = true;
      framed.lastSection = section;
      return section * framed.logical + offset;
    }
    return (framed.wroteThisFrame ? section : framed.lastSection) * framed.logical + offset;
  }
  ...
  createView(title, width, height) {
    ...
    this.views.set(pointer, {
      ...
      gesture: 0, closed: false, touchPoints: new Map(), pinchSpan: 0,
    });
  ...
  installViewEvents() {
    ...
    // Two fingers spreading or closing is a pinch on every other backend: iOS
    // recognizes one itself and a trackpad sends a ctrl-held wheel. A browser
    // only delivers the raw touches, so the factor is measured from the span
    // between them here and published through the same gesture state, leaving
    // the pointer stream untouched for whatever else owns those fingers.
    const pinch = (event, phase) => {
      const record = this.views.get(Number(this.activeView));
      if (!record) return;
      const id = Number(event.pointerId || 0);
      if (phase === 3 || phase === 4) {
        record.touchPoints.delete(id);
        record.pinchSpan = 0;
        return;
      }
      record.touchPoints.set(id, this.pointerPosition(event));
      if (record.touchPoints.size !== 2) {
        record.pinchSpan = 0;
        return;
      }
      const points = Array.from(record.touchPoints.values());
      const span = Math.hypot(points[0][0] - points[1][0], points[0][1] - points[1][1]);
      const previous = record.pinchSpan;
      record.pinchSpan = span;
      // The first frame with two fingers down only measures: a factor needs a
      // span to compare against.
      if (previous > 0 && span > 0) {
        this.viewImport("view_on_gesture", [this.activeView, 0, 0, span / previous, 0]);
      }
    };
    canvas.addEventListener?.("pointermove", event => {
      ...
      if (event.pointerType === "touch") pinch(event, 2);
    });
    ...
    canvas.addEventListener?.("pointerdown", event => {
      ...
      if (event.pointerType === "mouse") button(event, 0);
        ...
        if (event.pointerType === "touch") pinch(event, 1);
        canvas.focus?.();
      }
    });
    const release = (event, phase) => {
      if (event.pointerType === "mouse") button(event, 1);
        ...
        if (event.pointerType === "touch") pinch(event, phase);
      }
    ...
    // A browser reports how far the content should move; AppKit and Win32
    // report how far the wheel turned, which is the opposite sign. Negating
    // here leaves scroll_x and scroll_y meaning one thing on every backend:
    // positive is the wheel turning away from the hand.
    const wheelPixels = (delta, mode) => {
      if (mode === 1) return delta * VIEW_WHEEL_LINE_PIXELS;
      if (mode === 2) return delta * (this.canvas?.clientHeight || VIEW_WHEEL_PAGE_PIXELS);
      return delta;
    };
    canvas.addEventListener?.("wheel", event => {
      if (this.activeView) {
        const mode = Number(event.deltaMode) || 0;
        const dx = wheelPixels(Number(event.deltaX) || 0, mode);
        const dy = wheelPixels(Number(event.deltaY) || 0, mode);
        // A trackpad pinch arrives as a ctrl-held wheel and as nothing else, so
        // it becomes the gesture zoom the native backends publish rather than a
        // scroll the application would read as a wheel.
        if (event.ctrlKey) {
          this.viewImport("view_on_gesture", [this.activeView, 0, 0, Math.pow(2, -dy / VIEW_WHEEL_PINCH_PIXELS), 0]);
        } else {
          this.viewImport("view_on_scroll", [this.activeView, -dx, -dy]);
        }
      }
  ...
  viewImport(name, a) {
    ...
    if (name === "view_set_fullscreen") {
      // The page owns fullscreen; a request without a user gesture is rejected,
      // so a startup call is best-effort rather than an error.
      const on = Number(a[1]) !== 0, root = globalThis.document?.documentElement;
      if (!root) return;
      if (on && root.requestFullscreen) root.requestFullscreen().catch(() => {});
      else if (!on && globalThis.document.exitFullscreen) globalThis.document.exitFullscreen().catch(() => {});
      return;
    }
  ...
  async preloadFiles(paths = []) {
    for (const path of paths) {
      ...
      if (/\.(jpg|jpeg|png|webp)$/i.test(path)) await this.predecodeImage(path.replace(/^\.\//, ""), bytes);
    }
  }
  ...
  // lib/io.ns::io_image - three counts and the pixel array's handle - expanded
  // to the four channels every consumer here works in.
  readIOImage(pointer) {
    if (!pointer || !this.memory) return null;
    const view = this.view();
    const width = view.getInt32(Number(pointer), true);
    const height = view.getInt32(Number(pointer) + 4, true);
    const channels = view.getInt32(Number(pointer) + 8, true);
    if (width <= 0 || height <= 0 || channels < 1 || channels > 4) return null;
    const source = this.readBytes(view.getUint32(Number(pointer) + 12, true));
    const pixels = width * height;
    if (source.length < pixels * channels) return null;
    const rgba = new Uint8Array(pixels * 4);
    for (let index = 0; index < pixels; index++) {
      const base = index * channels;
      rgba[index * 4] = source[base];
      rgba[index * 4 + 1] = channels > 1 ? source[base + 1] : source[base];
      rgba[index * 4 + 2] = channels > 2 ? source[base + 2] : source[base];
      rgba[index * 4 + 3] = channels > 3 ? source[base + 3] : 255;
    }
    return { width, height, rgba };
  }

  // A decoded image as something drawImage takes. A canvas is not an <img>, so
  // it carries the two properties the paint path tests before it draws.
  imageFromPixels(image) {
  ...
  async predecodeImage(path, bytes) {
  ...
  wasm(name, a) {
    ...
    // A float in a format string prints with two decimals, as `ns run` does.
    if (name === "ftos") return this.writeString(Number(a[0]).toFixed(2));
  ...
  io(name, a) {
    ...
    // A browser has no encoder to write a PNG with and nowhere to put the file
    // if it had one, so the image is kept decoded under the path it was saved
    // to. That is the form the one reader of a saved image wants anyway:
    // ui_atlas_load picks it up from here, which is what lets a program that
    // rasterizes its own icons and loads them back as an atlas - a round trip
    // through the file system on a desktop - work unchanged in a tab.
    if (name === "io_load_image") {
      const path = this.readString(a[0]);
      const decoded = this.decodedImages.get(path);
      if (!path || !decoded) return 0;
      const pointer = this.allocStruct(16);
      const view = this.view();
      view.setInt32(pointer, decoded.width, true);
      view.setInt32(pointer + 4, decoded.height, true);
      view.setInt32(pointer + 8, 4, true);
      this.loadedImages.set(pointer, decoded);
      return pointer;
    }
    if (name === "io_image_copy_data") {
      const decoded = this.loadedImages.get(Number(a[0]));
      const pixels = decoded?.rgba || new Uint8Array();
      return this.writeArray(a[1], pixels, "u8", Number(a[2]));
    }
    if (name === "io_image_destroy") {
      this.loadedImages.delete(Number(a[0]));
      return;
    }
    if (name === "io_save_image") {
      const path = this.readString(a[0]);
      const image = this.readIOImage(a[1]);
      if (!path || !image) return 0;
      this.decodedImages.set(path, image);
      return 1;
    }
    if (name === "io_glb_read") {
  ...
  os(name, a) {
    ...
    // A browser bundle is served fresh on every load and never takes patches.
    if (name === "os_patch_version") return 0;
  ...
  ui(name, a) {
    ...
    // A page draws into one canvas: there is no immersive panel to project the
    // overlay onto and no head pose to aim at it, so the gaze is never active
    // and reports the middle of the drawable, as it does on a desktop window.
    if (name === "ui_hud_gaze_active") return 0;
    if (name === "ui_hud_gaze_x") return this.uiSurfaceSize(a[0])[0] * 0.5;
    if (name === "ui_hud_gaze_y") return this.uiSurfaceSize(a[0])[1] * 0.5;
    ...
    if (name === "ui_atlas_load") {
      ...
      if (!path) return 0;
      // An image the program itself produced and "saved" is already decoded, so
      // it becomes an atlas here and now: a caller that checks the size it just
      // loaded gets an answer on the same call rather than several frames later.
      const decoded = this.decodedImages.get(path);
      if (decoded) {
        const canvas = this.imageFromPixels(decoded);
        if (!canvas) return 0;
        const id = this.nextUIAtlas++;
        this.uiAtlases.set(id, { image: canvas });
        return id;
      }
  ...
  rootBinding() {
    ...
    const framedOffset = this.frameOffset(id, offset, false);
    // The block the shader declares, or the rest of the buffer when the program
    // bound a root of its own that is smaller than one; a shader reading past
    // what it was given is the program's own bug either way.
    const size = Math.min(ROOT_BLOCK_BYTES, Math.max(0, (buffer.size || ROOT_BLOCK_BYTES) - framedOffset));
    return { buffer, offset: framedOffset, size: align(Math.max(size, 16), 16) };
  }
  ...
  bindShaderResources(pass, pipeline, shader) {
    ...
    for (const slot of shader.storageSlots || []) {
      ...
      const buffer = this.get(id, "buffer");
      if (!buffer) return;
      entries.push({ binding: STORAGE_BINDING_BASE + slot, resource: { buffer, offset: this.frameOffset(id, offset, false) } });
    }
  ...
  gpu(name, a) {
    ...
    if (name === "gpu_commit") {
      ...
      this.commandEncoder = null;
      for (const framed of this.framedBuffers.values()) framed.wroteThisFrame = false;
      this.frameIndex++;
    ...
    if (name === "gpu_malloc") {
      ...
      if (!label) return 0n;
      const logical = Number(a[0]);
      const frames = (Number(a[1]) & 2) !== 0;
      const id = this.createBuffer(frames ? logical * 3 : logical, label);
      if (frames) this.framedBuffers.set(id, { logical, lastSection: 0, wroteThisFrame: false });
    ...
    if (name === "gpu_frame_alloc") {
      ...
      if (!this.frameBuffers[ring]) {
        ...
        this.frameBuffers[ring] = { id: this.put("buffer", this.device.createBuffer({ label, size: FRAME_RING_BYTES, usage })), offset: 0 };
      }
      ...
      if (frame.offset > FRAME_RING_BYTES) return 0n;
      return (BigInt(frame.id) << 32n) | BigInt(offset);
    }
    if (name === "gpu_free") {
      const { id } = this.addressParts(a[0]);
      this.get(id, "buffer")?.destroy();
      this.framedBuffers.delete(id);
      this.drop(id);
      return;
    }
    if (name === "gpu_write") {
      const { id, offset } = this.addressParts(a[0]);
      const buffer = this.get(id, "buffer");
      if (buffer && this.device) this.device.queue.writeBuffer(buffer, this.frameOffset(id, offset, true), this.readBytes(a[1], Number(a[2]))); return;
    }
    ...
    if (name === "gpu_set_root_data") {
      ...
      // The whole declared block is reserved even when the pass uploads less of
      // it, because the bind group covers the block the shader declares and a
      // range running past the ring would be refused.
      const address = this.gpu("gpu_frame_alloc", [Math.max(Number(a[1]), ROOT_BLOCK_BYTES), 256]);
      if (address) {
