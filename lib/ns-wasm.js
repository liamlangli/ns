...
class NSBrowserRuntime {
  ...
  createView(title, width, height) {
    ...
    this.closed = false;
    if (globalThis.document) document.title = this.readString(title) || document.title;
    if (this.canvas) {
  ...
  installViewEvents() {
    ...
    canvas.addEventListener?.("pointerdown", event => button(event, 0));
    canvas.addEventListener?.("pointerup", event => button(event, 1));
    canvas.addEventListener?.("pointercancel", event => button(event, 1));
