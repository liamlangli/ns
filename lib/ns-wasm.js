...
class NSBrowserRuntime {
  ...
  viewImport(name, a) {
    ...
    // application asking for a redraw burst or a delayed redraw is already served.
    if (name === "view_request_frame" || name === "view_request_frame_after") return;
  ...
  ui(name, a) {
    ...
    if (name === "ui_flush" || name === "ui_flush_overlay") {
      ...
      context.fillStyle = clear
        ? `rgba(${channel(0)},${channel(8)},${channel(16)},${clamp(this.view().getFloat64(clear + 24, true), 0, 1)})`
        : "rgba(18,20,23,1)";
    ...
    if (name === "ui_button") {
      ...
      if (widgets.input.mouseReleased) widgets.activeId = 0;
      return clicked ? 1 : 0;
    }
    if (name === "ui_slider") {
      const widgets = this.uiWidgets(a[0]);
