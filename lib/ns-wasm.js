...
class NSBrowserRuntime {
  ...
  gpu(name, a) {
    ...
    if (name === "gpu_pass_begin") {
      ...
      const colorFormats = [];
      for (let i = 0; i < 4; i++) {
        const texture = this.texture(a[i]);
        const view = texture?.texture.createView();
        if (view) {
          colorAttachments.push({ view, clearValue: { r: a[6], g: a[7], b: a[8], a: a[9] }, loadOp: ((Number(a[5]) >> (i * 2)) & 3) === 1 ? "load" : "clear", storeOp: "store" });
          colorFormats.push(texture.format);
        }
      }
      const depthTexture = this.texture(a[4]);
