...
class NSBrowserRuntime {
  ...
  bindShaderResources(pass, pipeline, shader) {
    ...
    if (shader.usesStorage && this.currentStorage) {
      const { id, offset } = this.addressParts(this.currentStorage);
      const buffer = this.get(id, "buffer");
      if (!buffer) return;
  ...
  gpu(name, a) {
    ...
    if (name === "gpu_shader_graphics_create") {
      ...
      return this.put("shader", {
        ...
        usesStorage: source.includes("ns_storage_buffer"),
    ...
    if (name === "gpu_set_storage") { this.currentStorage = BigInt(a[0]); return; }
