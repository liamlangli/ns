...
class NSBrowserRuntime {
  ...
  bindShaderResources(pass, pipeline, shader) {
    ...
    if (shader.usesStorage0 && this.currentStorages?.[0]) {
      const { id, offset } = this.addressParts(this.currentStorages[0]);
      const buffer = this.get(id, "buffer");
      if (!buffer) return;
      entries.push({ binding: 3, resource: { buffer, offset } });
    }
    if (shader.usesStorage1 && this.currentStorages?.[1]) {
      const { id, offset } = this.addressParts(this.currentStorages[1]);
      const buffer = this.get(id, "buffer");
      if (!buffer) return;
      entries.push({ binding: 7, resource: { buffer, offset } });
    }
  ...
  gpu(name, a) {
    ...
    if (name === "gpu_shader_graphics_create") {
      ...
      return this.put("shader", {
        ...
        usesStorage0: source.includes("ns_storage_buffer_0"),
        usesStorage1: source.includes("ns_storage_buffer_1"),
    ...
    if (name === "gpu_set_storage_at") {
      this.currentStorages ||= [];
      this.currentStorages[Number(a[0])] = BigInt(a[1]);
      return;
    }
