...
// A statically compiled call of one native function: `target` called with
// its arguments taken from `args` (a str, ref or array already the host
// pointer, a float as its bit pattern), returning the result the same way.
typedef u64 (*ns_cpu_native_call)(void *target, const u64 *args);

// Resolve a `ref fn` to a call shim and, in *target, the function it calls.
// Return NULL to fall back to `resolve` and libffi. Hosts without libffi (iOS
// apps generated by `ns project`) resolve through the shims of
// src/ns_embedded_ffi.c, which cover every signature.
typedef ns_cpu_native_call (*ns_cpu_resolve_call_fn)(void *user, const char *module, const char *name, void **target);

typedef struct ns_cpu_host {
    ns_cpu_resolve_fn resolve;
    ns_cpu_resolve_call_fn resolve_call;
    void *user;
