...
ssh "$host" "mkdir -p ${remote}/src ${remote}/include ${remote}/lib/include ${remote}/lib/src ${remote}/third_party/zlib ${remote}/third_party/zstd/lib ${remote}/third_party/box3d ${remote}/scripts"
...
rsync -a "${root}/src/ns_appimage_runtime.c" "${host}:${remote}/src/ns_appimage_runtime.c"
...
rsync -a "${host}:${remote}/bin/linux-x86_64/"*.so "${host}:${remote}/bin/linux-x86_64/ns-appimage-runtime" \
