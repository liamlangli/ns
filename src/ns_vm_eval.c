...
// NS_VALUE_TYPE ns_token_type(ns_token_t t) {
//     switch (t.type) {
...
// ns_value ns_eval_expr(ns_vm *vm, int i);


// int ns_vm_find_fn_arg_slot(ns_fn *fn, ns_str name) {
...
// int ns_vm_find_fn_local_slot(ns_fn *fn, ns_str name) {
...
// void ns_vm_parse_fn_expr(ns_vm *vm, int i, ns_fn *fn) {
//     if (i == -1)
//         return;
//     ns_ast_t n = vm->ast[i];
//     switch (n.type) {
...
// void ns_vm_parse_fn(ns_vm *vm, ns_ast_t n) {
...
// ns_value ns_eval_literal(ns_vm *vm, ns_ast_t n) {
...
//             // ns_fn *fn = &vm->fns[scope->fn_index];
...
// void ns_vm_parse_ast(ns_vm *vm, ns_ast_ctx *ctx) {
...
//         switch (n.type) {
//         case NS_AST_FN_DEF: {
//             ns_fn f = {.name = n.fn_def.name.val, .ast_root = n.fn_def.body};
...
// ns_value ns_new_bool(ns_vm *vm, bool value) {
//     ns_value v = {.type = NS_TYPE_BOOL, .u.boolean = value};
//     int i = ns_add_value(vm, v);
//     return vm->values[i];
// }

// ns_value ns_new_i32(ns_vm *vm, i32 value) {
//     ns_value v = {.type = NS_TYPE_I64, .u.int64 = value};
//     int i = ns_add_value(vm, v);
//     return vm->values[i];
// }

// ns_value ns_new_f64(ns_vm *vm, f64 value) {
//     ns_value v = {.type = NS_TYPE_F64, .u.float64 = value};
//     int i = ns_add_value(vm, v);
//     return vm->values[i];
// }

// int ns_vm_find_global_value(ns_vm *vm, ns_str name) {
...
// bool ns_vm_push_call_scope(ns_vm *vm, ns_call_scope scope) {
...
// ns_value ns_eval_binary_op(ns_vm *vm, ns_value left, ns_value right, int i) {
//     ns_ast_t n = vm->ast[i];
...
// ns_value ns_call_builtin_fn(ns_vm *vm, ns_str name, int i) {
//     ns_ast_t n = vm->ast[i];
...
// ns_value ns_call_fn(ns_vm *vm, int i) {
//     ns_ast_t n = vm->ast[i];
...
// ns_value ns_eval_jump_stmt(ns_vm *vm, int i) {
//     ns_ast_t n = vm->ast[i];
