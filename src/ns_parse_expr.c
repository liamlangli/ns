...
bool ns_parse_unary_expr(ns_parse_context_t *ctx) {
    ...
    if (ctx->token.type == NS_TOKEN_BIT_INVERT_OPERATOR ||
        ...
        if (ns_parse_postfix_expr(ctx)) {
            ...
            ns_parse_dump_error(ctx, "syntax error: expected expression after unary operator");
        }
    }
...
bool ns_parse_expr_stack(ns_parse_context_t *ctx) {
    ...
    do {
        ...
        case NS_TOKEN_LOGICAL_OPERATOR: {
            ...
            if (ctx->top == expr_top) {
                ...
                if (ns_parse_unary_expr(ctx)) {
                    ...
                    ns_parse_dump_error(ctx, "syntax error: expected unary expression");
                }
            }
        ...
        default:
            ns_parse_dump_error(ctx, "fatal error: unexpected token type in expr stack parser.");
            break;
        }
