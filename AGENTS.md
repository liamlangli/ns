...
| `compress` | Byte compression backed by pinned zlib 1.3.1 sources (doc/compress.md). Three framings, each with its own bound/encode/decode trio: raw deflate, zlib, and gzip. The surface is allocation-free — the caller owns both buffers and every call returns the bytes produced or a negative `COMPRESS_ERROR_*` status. Only gzip records a decoded size. Also exposes zlib's seeded `crc32`/`adler32`. Native desktop and generated Apple apps are supported. |
