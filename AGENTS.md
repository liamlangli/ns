...
| `view` | Application window/view lifecycle plus keyboard, pointer, gesture, clipboard, frame callback, size, scale, and GPU-device state. Wasm projects use the generated HTML canvas as the view backend; Linux uses an xdg-shell Wayland toplevel and a host without a compositor gets a no-op view. |
| `gpu` | Platform GPU access. The v2-only Nano Script surface (doc/gpu.md) treats the GPU as a processor with memory: 64-bit addresses from `gpu_malloc`, bindless u32 texture/sampler indices, one root pointer per draw/dispatch, and a triple-buffered frame ring for transient data. Per-frame CPU writes use `gpu_set_root_data` / `gpu_frame_write` or `GPU_MEM_FRAMES`; a single mapped buffer rewritten every frame races the in-flight GPU. It runs host-side headless. Apple uses Metal, Windows uses DirectX 12, Linux uses Vulkan with shaderc-compiled GLSL, and unsupported backends may return failure. |
| `ui` | Immediate-mode native UI rendering, layout, themes, input snapshots, widgets, scrolling, text editing/views, images, and renderer primitives on top of `view` and `gpu`. Wasm projects get every entry point the native library implements, drawn through a Canvas 2D backend in `ns-wasm.js` (doc/wasm.md); glyphs come from the page font stack there, so a face load fails the way a missing atlas does natively. |
| `compress` | Byte compression backed by zlib and pinned Zstandard 1.5.7 sources (doc/compress.md). Four framings, each with its own bound/encode/decode trio: raw deflate, zlib, gzip, and zstd. The surface is allocation-free — the caller owns both buffers and every call returns the bytes produced or a negative `COMPRESS_ERROR_*` status. Only gzip and zstd record a decoded size. Also exposes zlib's seeded `crc32`/`adler32`. Native desktop and generated Apple apps are supported. |
| `dynamic` | Native 3D rigid-body simulation backed by a pinned Box3D v0.1.0 download (doc/dynamic.md). The Nano Script wrapper keeps plain body/config structs and a host state mirror while `dynamic.dylib`/`dynamic.so` owns broad phase, collision manifolds, continuous collision, and the Soft Step solver. One hull point plus a margin maps to a sphere, two points plus a margin map to a capsule, and larger point sets become Box3D convex hulls. Native desktop only; Wasm and generated Apple IDE apps reject this external FFI module. |
...
- Give every GPU buffer allocation a stable, concrete, purpose-specific debug
  ...
  than the call.
- Default per-frame GPU data to triple buffering. Small uniforms go through
  `gpu_set_root_data`; other CPU-written dynamic buffers use `gpu_frame_write`
