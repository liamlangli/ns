...
function make_font_metrics(font_type = FONT_MAIN) {
    return {
        font_type,
        glyph_w: 8,
        glyph_h: 18,
        get_glyph() {
            return { xoff: 0, yoff: 0, w: 8, h: 18, u0: 0, v0: 0, u1: 1, v1: 1 };
        },
    };
}

class webgpu_draw_adapter {
    constructor(renderer, get_viewport) {
        this.renderer = renderer;
        this.get_viewport = get_viewport;
        this._clip = false;
    }

    get s() { return window.devicePixelRatio || 1; }

    clear() {
        if (this._clip) {
            this.renderer.pop_clip();
            this._clip = false;
        }
    ...
    rect(x, y, w, h, r, g, b, a = 1) {
        const s = this.s;
        this.renderer.fill_rect(x * s, y * s, w * s, h * s, pack_rgba(r, g, b, a));
    }

    fill_convex_poly(pts, r, g, b, a = 1) {
        if (pts.length < 3) return;
        const s = this.s;
        const color = pack_rgba(r, g, b, a);
        const [x0, y0] = pts[0];
        for (let i = 1; i < pts.length - 1; i++) {
            const [x1, y1] = pts[i];
            const [x2, y2] = pts[i + 1];
            this.renderer.fill_triangle(x0 * s, y0 * s, x1 * s, y1 * s, x2 * s, y2 * s, color);
        }
    }

    text(str, x, y, font, r, g, b, a = 1) {
        const s = this.s;
        const font_type = font?.font_type ?? FONT_MAIN;
        const target_line_h = (font?.glyph_h ?? 18) * s;
        // Calibrate font_px so the renderer's actual line height (which includes
        // its internal default_font_scale and atlas line_height/font_size ratio)
        // matches glyph_h.
        const unit_line_h = this.renderer.text_line_height(1, font_type);
        const size = unit_line_h > 0 ? target_line_h / unit_line_h : target_line_h;
        this.renderer.draw_text(x * s, y * s, str, size, pack_rgba(r, g, b, a), font_type);
        return x + str.length * (font?.glyph_w ?? 8);
    }

    text_clipped(str, x, y, max_w, font, r, g, b, a = 1) {
        const glyph_w = font?.glyph_w ?? 8;
        const visible = str.slice(0, Math.max(0, Math.floor(max_w / glyph_w)));
        return this.text(visible, x, y, font, r, g, b, a);
    }

    glyph(x, y, w, h, uv, r, g, b, a = 1) {
        this.rect(x, y, w, h, r, g, b, a);
    }

    scissor(x, y, w, h) {
        if (this._clip) this.renderer.pop_clip();
        const s = this.s;
        const vp = this.get_viewport();
        const full = x <= 0 && y <= 0 && w >= vp.w && h >= vp.h;
        if (full) {
            this._clip = false;
            return;
        }
        this.renderer.push_clip(x * s, y * s, w * s, h * s);
        this._clip = true;
    }
}
...
export class UI {
    constructor(canvas) {
        this._canvas = canvas;
        this._renderer = canvas ? new ui_renderer(canvas) : null;
        this._font     = make_font_metrics(FONT_MAIN);
        this._code_font = make_font_metrics(FONT_MONO);
    ...
    // ── Dock chrome ───────────────────────────────────────────────────────────
    /**
     * Drive @liamlangli/ui's `dock_system` for one frame — this is the *whole*
     * dock chrome (rounded panels, per-corner tabs, accent indicator, close
     * affordance, dirty dots, splitters, drag ghost, drop overlay), shared with
     * the upstream `ui` preview so NSCode's dock looks identical.
     *
     * `dock` is a `dock_system` instance (owns the layout + drag/resize state).
     * `x,y,w,h` are logical px (scaled here); `input.mouse_*` must already be in
     * physical px (mx * scale). `render_body(panel)` fills each visible panel —
     * `panel.{x,y,w,h}` come back in physical px.
     *
     * `dock_system` requests resize cursors via `renderer.set_cursor`, but
     * NSCode's `render()` applies the accumulated `_want_cursor` last (so the
     * last writer wins across passes). Capture what the dock asks for and fold
     * it into `_want_cursor` so the col-/row-resize feedback survives.
     */
    dock_frame(dock, input, x, y, w, h, render_body, opts) {
        if (!this._renderer) return;
        const s = this.scale;
        const r = this._renderer;
        const real_set_cursor = r.set_cursor.bind(r);
        let captured;
        r.set_cursor = (c) => { captured = c; };
        try {
            dock.frame(r, NS_THEME, input, x * s, y * s, w * s, h * s, render_body, opts);
        } finally {
            r.set_cursor = real_set_cursor;
        }
        if (captured != null) this._want_cursor = captured;
    }
    ...
    get mono_font() { return this._code_font; }
