...
const AST_RECORD_U32 = 6;   // functionId, tokenStart, tokenCount, nodeKind, payload0, payload1
const COUNTER_U32 = 4;      // tokenCount, astCount, tokenOverflow, astOverflow
...
export type FunctionSpanInput = { start: number; end: number };
...
export type ParserRunResult = {
    ...
    counters: { tokenCount: number; astCount: number; tokenOverflow: boolean; astOverflow: boolean };
    tokens: TokenRecord[];
    astNodes: AstNodeRecord[];
    astByFunction: AstNodeRecord[][];
};

function divUp(a: number, b: number): number {
    return Math.ceil(a / b);
}

function toU32Words(bytes: Uint8Array): Uint32Array {
...
export class GPUParser {
    ...
    async run(source: string, functionSpans: FunctionSpanInput[]): Promise<ParserRunResult> {
        const encoderUtf8 = new TextEncoder();
        const sourceBytes = encoderUtf8.encode(source);
        ...
        const astByFunction = Array.from({ length: spans.length }, () => [] as AstNodeRecord[]);
        for (const node of astNodes) {
            if (node.functionId >= astByFunction.length) continue;
            const bucket = astByFunction[node.functionId];
