...
A host can instead hand ns_cpu a statically compiled call shim per function
(`ns_cpu_host.resolve_call`): `src/ns_embedded_ffi.c`, generated by
`tools/gen_embedded_ffi.ns`, has one for every function of the modules an
Apple app embeds, and generated Xcode apps resolve through it, so an `emu` app
calls its native modules without libffi on iOS. Without libffi or a shim
(`NS_XCLIB` builds), a call with up to eight integer and eight float arguments
is made directly on AArch64 and x86-64 System V; a call with more faults when
it is reached.
...
shipped images links it alone with the ns_rt runtime. Apple apps generated by
`ns project` embed it this way (see doc/nsm.md): an app whose manifest sets
`target = "emu"` builds its image during the Xcode build and runs it with
`ns_cpu_run_main`.
